...
def test_voting_strategy_reasoning_task():
    ...
    engine.cast_vote(
        "RFC-001", "agent_bob", Vote.APPROVE, "Good approach, FL is proven technology"
    )
    engine.cast_vote(
        "RFC-001",
        "agent_charlie",
    ...
    engine.cast_vote("RFC-001", "agent_diana", Vote.APPROVE, "Benefits outweigh costs")
    ...
    assert (
        len(adr.provenance) == 3
    ), f"Should include proposer + 2 voters. Got {len(adr.provenance)}"
...
def test_consensus_strategy_knowledge_task():
    ...
    engine.cast_vote(
        "RFC-002", "agent_eve", Vote.APPROVE, "Definition aligns with complexity theory"
    )
    engine.cast_vote(
        "RFC-002", "agent_frank", Vote.APPROVE, "Consistent with our usage in code"
    )
    engine.cast_vote("RFC-002", "agent_grace", Vote.APPROVE, "Clear and unambiguous")
    engine.cast_vote("RFC-002", "agent_henry", Vote.ABSTAIN, "Should include examples")
...
def test_rejection_requires_rework():
    ...
    # Most agents reject
    engine.cast_vote(
        "RFC-003", "agent_alice", Vote.REJECT, "This would make system fragile"
    )
    engine.cast_vote(
        "RFC-003", "agent_bob", Vote.REJECT, "Violates production-ready requirements"
    )
    engine.cast_vote(
        "RFC-003",
        "agent_charlie",
