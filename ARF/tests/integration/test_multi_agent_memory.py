...
def alice_memory(temp_dir):
    ...
    memory = ConversationMemory(
        agent_id="alice", storage_path=str(storage_path), backend="file"
    )
    yield memory
...
def bob_memory(temp_dir):
    ...
    memory = ConversationMemory(
        agent_id="bob", storage_path=str(storage_path), backend="file"
    )
    yield memory
...
def carol_memory(temp_dir):
    ...
    memory = ConversationMemory(
        agent_id="carol", storage_path=str(storage_path), backend="file"
    )
    yield memory
...
def test_three_agent_composition(alice_memory, bob_memory, carol_memory):
    ...
    assert (
        len(recalls) >= 2
    ), "Carol should recall at least 2 understandings about Sonnet-4"
...
def test_multi_agent_deduplication(alice_memory, bob_memory):
    ...
    alice_memory.transmit({"content": content, "coherence": 0.9})

    bob_memory.transmit({"content": content, "coherence": 0.9})
...
def test_composition_with_embeddings(alice_memory, bob_memory):
    ...
    bob_memory.transmit(
        {"content": "Deep learning uses neural networks", "coherence": 0.85}
    )
...
def test_parallel_composition(temp_dir):
    ...
    for i in range(5):
        ...
        memory = ConversationMemory(
            agent_id=f"agent_{i}", storage_path=str(storage_path)
        )
    ...
    coordinator = ConversationMemory(
        agent_id="coordinator", storage_path=str(coordinator_path)
    )
...
def test_triple_extraction_consistency(alice_memory, bob_memory):
    ...
    bob_memory.transmit({"content": "Claude-4 is a LLM", "coherence": 0.95})
