...
class TestConversationMemoryComposition:
    ...
    def test_composition_with_embeddings(self):
        ...
        memory1 = ConversationMemory(
            agent_id="agent1", storage_path=f"{self.temp_dir}/agent1"
        )
        memory1.transmit(
            {"content": "Understanding from agent 1", "context": "Test context 1"}
        )

        memory2 = ConversationMemory(
            agent_id="agent2", storage_path=f"{self.temp_dir}/agent2"
        )
        memory2.transmit(
            {"content": "Understanding from agent 2", "context": "Test context 2"}
        )
    ...
    def test_composition_multiple_agents(self):
        ...
        human_memory = ConversationMemory(
            agent_id="human", storage_path=f"{self.temp_dir}/human"
        )
        ...
        ai1_memory = ConversationMemory(
            agent_id="ai1", storage_path=f"{self.temp_dir}/ai1"
        )
        ai1_memory.transmit(
            {"content": "AI 1 analysis of the situation", "context": "AI 1 perspective"}
        )

        ai2_memory = ConversationMemory(
            agent_id="ai2", storage_path=f"{self.temp_dir}/ai2"
        )
        ai2_memory.transmit(
            {"content": "AI 2 recommendation", "context": "AI 2 perspective"}
        )
    ...
    def test_composition_preserves_provenance(self):
        ...
        # ARRANGE
        memory1 = ConversationMemory(
            agent_id="originator", storage_path=f"{self.temp_dir}/originator"
        )
        memory1.transmit(
            {"content": "Original insight", "context": "From the originator"}
        )

        memory2 = ConversationMemory(
            agent_id="receiver", storage_path=f"{self.temp_dir}/receiver"
        )
    ...
    def test_composition_with_decisions(self):
        ...
        # ARRANGE
        memory1 = ConversationMemory(
            agent_id="decision_maker", storage_path=f"{self.temp_dir}/decision_maker"
        )
        memory1.transmit(
            {
        ...
        memory2 = ConversationMemory(
            agent_id="reviewer", storage_path=f"{self.temp_dir}/reviewer"
        )
        ...
        assert (
            memory2.adrs[0]["content"]["content"] == "Important architectural decision"
        )
    ...
    def test_composition_without_embeddings(self):
        ...
        memory1.transmit({"content": "Understanding without embeddings"})
    ...
    def test_composition_idempotency(self):
        ...
        memory1.transmit({"content": "Unique understanding"})
    ...
    def test_composition_persistence(self):
        ...
        memory1.transmit({"content": "Understanding 1"})

        memory2 = ConversationMemory(
            agent_id="agent2", storage_path=f"{self.temp_dir}/agent2"
        )
        memory2.transmit({"content": "Understanding 2"})
