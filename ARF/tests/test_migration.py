...
def test_migrate_understanding_success():
    ...
    memory = ConversationMemory(
        agent_id="test", storage_path="./test_temp", validate_ontology=True
    )
    ...
    assert (
        understanding["migration_metadata"]["extraction_method"] == "pattern_matching"
    )
...
def test_full_migration_script():
    ...
    try:
        ...
        memory.transmit(
            {"content": "GPT-4 is a large language model", "coherence": 0.9},
        ...
        result = subprocess.run(
            [
                sys.executable,
                str(
                    Path(__file__).parent.parent / "scripts" / "migrate_to_symbolic.py"
                ),
            ],
            capture_output=True,
            text=True,
