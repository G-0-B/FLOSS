...
from materialize_shared_context_surface import materialize as materialize_context_surface
...
def resolve_manifest(workspace_root: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(manifest.get("targets"), dict):
        raise SharedSurfaceError(f"{manifest_path} must contain an object-valued `targets` field")
    if not isinstance(manifest.get("mcp_source"), str) or not manifest["mcp_source"].strip():
        raise SharedSurfaceError(f"{manifest_path} must contain a non-empty `mcp_source`")
    if "context_pointers" in manifest and not isinstance(manifest["context_pointers"], list):
        raise SharedSurfaceError(f"{manifest_path} field `context_pointers` must be a list")

    source_path = workspace_root / manifest["mcp_source"]
    source_data = load_json(source_path)
    mcp_servers = source_data.get("mcpServers")
    if not isinstance(mcp_servers, dict):
        raise SharedSurfaceError(f"{source_path} must contain an object-valued `mcpServers` field")
...
def build_context_pack(manifest: dict[str, Any]) -> str:
    workspace_name = manifest.get("workspace_name", manifest.get("workspace_id", "workspace"))
...
def build_gemini_payload(existing: dict[str, Any], shared_mcp: dict[str, Any]) -> dict[str, Any]:
...
def build_opencode_payload(existing: dict[str, Any], shared_mcp: dict[str, Any]) -> dict[str, Any]:
    payload = dict(existing)
    existing_mcp = payload.get("mcp", {})
    if existing_mcp is None:
        existing_mcp = {}
    if not isinstance(existing_mcp, dict):
        raise SharedSurfaceError("OpenCode config field `mcp` must be an object if present")
...
def check_or_write(path: Path, payload: dict[str, Any], *, check: bool, dry_run: bool) -> tuple[str, bool]:
    changed = True
    if path.exists():
...
def check_or_write_text(path: Path, content: str, *, check: bool, dry_run: bool) -> tuple[str, bool]:
    changed = True
    if path.exists():
...
def materialize(workspace_root: Path, manifest_path: Path, *, check: bool, dry_run: bool) -> tuple[list[str], bool]:
    ...
    if isinstance(context_pack_path, str) and context_pack_path.strip():
        ...
        message, changed = check_or_write_text(path, content, check=check, dry_run=dry_run)
        results.append(message)
        drift_found = drift_found or changed
    ...
    if isinstance(gemini_cfg, dict) and gemini_cfg.get("settings_path"):
        ...
        message, changed = check_or_write(gemini_path, payload, check=check, dry_run=dry_run)
        results.append(message)
        drift_found = drift_found or changed
    ...
    if isinstance(opencode_cfg, dict) and opencode_cfg.get("settings_path"):
        ...
        message, changed = check_or_write(opencode_path, payload, check=check, dry_run=dry_run)
        results.append(message)
        drift_found = drift_found or changed
