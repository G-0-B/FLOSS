...
def validate_hook_scripts(manifest: dict[str, Any], workspace_root: Path) -> list[dict[str, str]]:
...
def merge_hook_payload(existing: dict[str, Any], target_cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    for event_name, definitions in target_hooks.items():
        if not isinstance(definitions, list):
            raise HookSurfaceError(f"Hook event {event_name!r} must contain a list of definitions")
    ...
    if target_hooks_config is not None:
        if not isinstance(target_hooks_config, dict):
            raise HookSurfaceError("Target `hooksConfig` field must be an object if present")
        ...
        if not isinstance(existing_hooks_config, dict):
            raise HookSurfaceError("Existing `hooksConfig` field must be an object if present")
...
def build_registry(manifest: dict[str, Any], workspace_root: Path) -> dict[str, Any]:
    ...
    for target_name, target_cfg in manifest["targets"].items():
        ...
        if isinstance(settings_path, str) and settings_path.strip():
            resolved_cfg["resolved_settings_path"] = str((workspace_root / settings_path).resolve())
...
def build_index(registry: dict[str, Any]) -> str:
    ...
    for target_name, target_cfg in registry.get("targets", {}).items():
        ...
        if isinstance(hooks_config, dict) and hooks_config:
            lines.append(f"- Hooks config keys: `{', '.join(sorted(hooks_config.keys()))}`")
        lines.append("")
...
def materialize(
    ...
    *,
    ...
    message, changed = check_or_write_text(index_path, index, check=check, dry_run=dry_run)
    results.append(message)
    drift_found = drift_found or changed
    ...
    for target_name, target_cfg in manifest["targets"].items():
        ...
        if not isinstance(settings_path, str) or not settings_path.strip():
            raise HookSurfaceError(f"Enabled target {target_name!r} must define `settings_path`")
        target_path = (workspace_root / settings_path).resolve()
        existing = load_jsonc(target_path) if target_path.exists() and target_path.suffix == ".jsonc" else (
            load_json(target_path) if target_path.exists() else {}
        )
        payload = merge_hook_payload(existing, target_cfg)
        message, changed = check_or_write_json(target_path, payload, check=check, dry_run=dry_run)
        results.append(message)
        drift_found = drift_found or changed
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Materialize the FLOSSI0ULLK shared hook surface")
