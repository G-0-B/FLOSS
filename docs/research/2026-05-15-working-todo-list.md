...
**Open items:** (a) service runner still undecided — §A.1 Servy recipe stands, but arguments must include the PYTHONPATH-clean env (Servy env field: `PYTHONPATH=`) and `C:\Python313\python.exe` explicitly; (b) `save_daily_state` only persists at end-of-tick — a mid-tick kill loses the tick count (minor; consider persisting after increment); (c) `loop_stdout.log` is a dead surface (frozen 2026-05-13, superseded by `ticks.log`) — archive or delete; (d) root operator-surface repo gc'd 6.8 GB → **55 MB** 2026-07-07 (`git gc --aggressive --prune=now`; only unreachable pre-gitignore blobs dropped) + `README-REPO.md` added at workspace root documenting the intentional two-repo topology so agents stop re-diagnosing it; (e) Anthony recalls an "antigravity" batch intake-processing script — not found under `FLOSS/scripts/` (only `process_intake_events.py` + `watch_intake.py` exist there; closest other artifact is `major_consolidation_sweep.py`, mtime 2026-07-05, an LLM corpus-consolidation sweeper) — locate or confirm gone.
