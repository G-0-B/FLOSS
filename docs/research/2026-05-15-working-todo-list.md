...
12. **ADR-7 relicense — DECLARED RESOLVED BUT NEVER EXECUTED.** *(Corrected 2026-08-11; the earlier wording of this item treated it as propagation bookkeeping and was wrong.)* `RESEARCH-REGISTER.md:49` states "**Resolved 2026-08-04** | Project relicensed **AGPL-3.0-or-later**". On disk, `FLOSS/LICENSE` is **GPL-3.0** — 674 lines, header "GNU GENERAL PUBLIC LICENSE / Version 3, 29 June 2007", zero AFFERO mentions in the title; the only three AFFERO occurrences are lines 552/556/559, which are GPLv3 §13 (the GPL→AGPL *combination* clause), not an AGPL grant. There is no `LICENSE` at workspace root at all. So the register marks resolved something that never landed.
    Consequences: (a) every downstream claim of AGPL — including in this session's kernel-merge commit and in `RESEARCH-REGISTER.md`'s own front-matter `license:` field — is unsupported; (b) reuse-gate license assessments computed against AGPL need redoing against GPL-3.0; (c) the register itself flags "conflicting completion claims" at line 157 and notes at line 161 that a relicense "requires all copyright holders to agree" — non-trivial given bot and multi-model contributions. **Decide and execute, or retract the resolution.** Not a new ADR either way.
