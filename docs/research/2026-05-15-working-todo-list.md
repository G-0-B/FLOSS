...
18. **Shadow directories — priority raised 2026-08-12.** `docs/ADRs/` holds `ADR-2-Holochain-Integration-Stack.md` and `ADR-3-Documentation-Consolidation.md`, which **collide by number** with the real ADR-2/ADR-3 in `docs/adr/`; `docs/specifications/` holds one stray `keri-identity-bridge.yaml`. Both unindexed by `adr/INDEX.md` and ungated by `spec_gate`.
    **Why this got worse:** the Codex repository atlas (`.toilet/2026-08-11-flossi0ullk-repo-atlas/FILE_INVENTORY.tsv`) classifies both shadow ADRs as `canon-governance` at read-priority **L1** — so any agent trusting that map loads them *as governance material*, with no signal that the numbers are already taken. A latent trap became a signposted one. Resolution is `archive/`: unlike root intake, these **are** superseded canonical docs, so archive is the correct destination.

18b. **`serena-memory` corpus is routed but not present.** `shared-context-surface.json` declares `serena-memory` (`floss://serena-memory`, tier L1, priority 80, "Repo-local Serena project configuration and durable codebase memories") and lists it in `default_route_order`. Verified 2026-08-12: `.serena/` has **0 tracked files** both at the atlas anchor `4657bdaa` and on PR #38's head branch, and **8 tracked files** only on `wip/salvage-20260804`. A fresh clone of PR #38 therefore routes agents to an empty corpus. The atlas flagged this and explicitly declined to resolve it ("records the drift for a separately authorized context-surface task"). Fix by landing `.serena/` on the integration branch, or by removing the corpus from the manifest — but not by leaving the manifest advertising something that is not there.
