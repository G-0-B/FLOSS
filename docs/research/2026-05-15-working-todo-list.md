...
12. **ADR-7 is decided and ratified; only `FLOSS/LICENSE` was never updated.** *(Corrected twice. The 2026-08-11 wording said "declared resolved but never executed", implying no decision record existed. That was wrong — and it inverted the consequence.)*
    ...
    **Action:** replace `FLOSS/LICENSE` with the AGPL-3.0 text (or the SPDX one-liner, matching FLOSSI_U), then flip ADR-7's Truth Status to Verified. Operator decision — a license file change is not an agent's unilateral call. Note ADR-7 §3 also reserves a dual-licensing/steward-vote carve-out that should survive into whatever text lands.
