...
| **N5** | ✅ ADOPTED + BUILT same day (Anthony: "both", 2026-06-12). `scripts/spec_gate.py` + `docs/specs/spec-registry.json` (79 entries) + schema + spec. Fail-closed `--check` audit + advisory in `hook_post_write.py` additionalContext. Scope: canon surfaces only — intake mouths, continuation seeds, and agent-memory are never gated. |
| **N6** | ✅ GO + LANDED (Anthony, 2026-06-12). `scripts/objectgraph_spike.py` over skill-corpus: 14 docs → 112 nodes, 182 edges; idempotent build; known-answer retrieval verified; both risk guards live (synthesis advisory + provenance paths); 6/6 tests. Spec: `docs/superpowers/specs/2026-06-12-objectgraph-spike.md`. |
