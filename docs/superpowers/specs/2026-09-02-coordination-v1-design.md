...
- **Authority note:** every operation validates the finished ref using `git check-ref-format` — necessary but NOT sufficient on Windows. Raw `scripts/foo.lock`, `docs/a b.md`, `docs/x..y.md`, `C:/other/foo.py` are illegal as ref names (verified 2026-09-03); outside-repo path claim → no id (fail closed). Reserved device basenames as exact encoded components (`CON`/`PRN`/`AUX`/`NUL`/`COM1-9`/`LPT1-9`, case-insensitive — e.g. branch `aux`, path `docs/con`) pass check-ref-format yet fail `update-ref` with `Invalid argument`, so `claim_ref` rejects them explicitly. Names that merely contain those strings or carry them before an encoded dot (`con%2Emd` for `con.md`, `console%2Emd`) are legal — verified live: `con%2Emd` rc=0, bare `aux` rc=128 (M1).
- **Illegal id vs conflict:** `claim()` returns `(False, "E_ILLEGAL_ID")` when `check-ref-format` fails, a reserved device component is hit, or `repo_relative_path` returns `None`. Never report that as `conflict`. `is_claim_blocked()` **fails closed** (treat as blocked / raise) if it cannot form a legal ref — never `(False, "")` meaning "not claimed".
