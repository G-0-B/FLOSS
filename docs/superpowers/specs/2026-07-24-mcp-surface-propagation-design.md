...
## Known gap: the Claude Desktop app injects its own agentmemory server (2026-07-25)

**`.mcp.json` is the single source of truth for the six FILE-based harness surfaces. It is not the source of truth for Claude Desktop sessions.**

The Claude Desktop app spawns `claude.exe` with an inline MCP config on the command line:

```
--mcp-config "{\"mcpServers\":{\"agentmemory\":{\"type\":\"stdio\",\"command\":\"cmd\",
  \"args\":[\"/c\",\"npx\",\"-y\",\"@agentmemory/mcp\"],
  \"env\":{\"AGENTMEMORY_URL\":\"${AGENTMEMORY_URL}\",\"AGENTMEMORY_SECRET\":\"${AGENTMEMORY_SECRET}\"}}}}"
```

Verified empirically 2026-07-25: called `memory_diagnose` from a Desktop session and the JanuScope audit sink (`.agent-surface/heartbeat/janus-agentmemory-audit.jsonl`) did not grow by a single byte, while the call succeeded. The server name collides with the `agentmemory` entry in root `.mcp.json`, and the CLI flag wins.

Consequences in Desktop sessions:
- **No audit trail** — lens-level logging is bypassed.
- **No instruction injection** — the lens's invariants (Plane-A-not-canon, no-secrets) never reach the model.
- **`npx -y` per session** — the spawn pattern eliminated from every other surface.

**This surface is unreachable by the propagator** — it is a CLI argument generated by the app, not a file any manifest controls. Fixing it requires changing the agentmemory connector configuration in the Claude Desktop app itself, which would then let the lens-wrapped `.mcp.json` entry take effect. Recorded here so the "single source of truth" claim above is not read more broadly than it holds.
