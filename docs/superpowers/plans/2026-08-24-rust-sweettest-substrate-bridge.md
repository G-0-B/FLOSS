...
**Architecture:** Add one native-only Rust test package at `ARF/tests/sweettest`. Two integration-test binaries share typed wire representations and a two-conductor fixture but create independent conductor state; production zomes remain unchanged.

**Tech Stack:** Rust 2021, Holochain/Sweettest `=0.6.1`, HDK `=0.6.1`, Tokio multi-thread runtime, Serde, Holonix `main-0.6`.
...
- Modify `ARF/Cargo.toml`: register `tests/sweettest` as the fifth workspace member.
- Modify `ARF/Cargo.lock`: record exact native Sweettest dependency resolution generated by Cargo.
- Create `ARF/tests/sweettest/Cargo.toml`: native test-package dependency boundary.
...
ReviewZome result: no zome source file changes are planned. Sweettest requirements are satisfied by `#[tokio::test(flavor = "multi_thread")]`, `holochain` with `test_utils`, peer exchange, and `await_consistency` before cross-agent reads. No ReviewZome blocker applies.
...
**Files:**
- Modify: `ARF/Cargo.toml`
- Modify: `ARF/Cargo.lock`
...
```bash
nix develop path:. --command cargo test -p rose_forest_sweettest --test substrate_bridge_test criterion_1_publish_returns_resolvable_action_hash -- --exact
```

Expected: FAIL because package `rose_forest_sweettest` is not yet registered. This proves the new native harness is absent.

- [ ] **Step 3: Add the test package and workspace member**

Append `"tests/sweettest"` to the workspace `members` array. Create `ARF/tests/sweettest/Cargo.toml`:
...
```bash
nix develop path:. --command cargo build --workspace --exclude rose_forest_sweettest --release --target wasm32-unknown-unknown
...
```bash
git add ARF/Cargo.toml ARF/Cargo.lock ARF/tests/sweettest
...
```bash
nix develop path:. --command cargo test -p rose_forest_sweettest --test substrate_bridge_test
```
...
```bash
nix develop path:. --command cargo test -p rose_forest_sweettest --test consent_zome_test
```
...
**Files:**
- Modify only if generated resolution changed: `ARF/Cargo.lock`
...
```bash
nix develop path:. --command hc dna pack dnas/rose_forest/workdir/
nix develop path:. --command cargo test -p rose_forest_sweettest --test substrate_bridge_test --test consent_zome_test
nix develop path:. --command cargo test -p rose_forest_sweettest --test substrate_bridge_test --test consent_zome_test
```

Expected: fresh pack succeeds and both consecutive test runs pass with identical test counts.
...
Expected: original 40 unit tests plus new integration tests pass.
...
Expected changed paths: design/plan docs, `ARF/Cargo.toml`, `ARF/Cargo.lock`, and `ARF/tests/sweettest/**`. The packed `.dna` artifact should not be committed unless it was already tracked and changed by the required pack; if changed, report it separately and leave it unstaged pending operator direction.

- [ ] **Step 6: Commit final generated resolution if needed**

If `ARF/Cargo.lock` changed after the last feature commit:

```bash
git add ARF/Cargo.lock
