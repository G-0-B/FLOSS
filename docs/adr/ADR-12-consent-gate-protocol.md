...
**Currently NOT verified end-to-end:** Integration scenarios for `consent_gate.test.ts` do not run. `@holochain/tryorama@^0.19.0` + `@holochain/client@^0.20.0` send a request shape `holochain conductor 0.6.1` cannot deserialize (`AdminWebsocket.installApp` -> `deserialization: Failed to deserialize request`). **Tooling issue, NOT a consent zome defect** — the wasm builds, the manifest packs, the validation logic is unit-tested at 10/10.

**Corrected 2026-08-17 — the previously stated unblock was a dead end.** This ADR formerly said the fix was "pinning compatible client+tryorama versions for the 0.6 conductor line" (M13). That path does not exist and cannot be made to exist:

- **No Tryorama version pairs with hc 0.6.1.** Investigated 2026-05-26 and recorded in `ARF/tests/tryorama/package.json`: 0.17 wants an `hc-sandbox` binary absent from 0.6.x; 0.18 wants `hc sandbox network webrtc` + `ConfigRootPath(...)` (0.6.1 has `mem`/`quic` + `DataRootPath`); 0.19 sends the undeserializable shape above.
- **Tryorama has been retired upstream** as the official test-authoring framework in favour of Rust `sweettest`; support for Holochain 0.7+ is explicitly not to be expected. Only a community fork remains (`holochain-open-dev/tryorama`).
- Pinning a retired framework to a line the repo itself calls EOL upstream is not a route to Accepted.

**Actual unblock: port `consent_gate.test.ts` to `sweettest`.** Verified 2026-08-17 that `holochain 0.6.3` exposes the `sweettest` module (`SweetConductor`, `SweetDnaFile`, `SweetConductorBatch`, `SweetInlineZomes`), so this is available on the current 0.6 line **without** a substrate migration. Sweettest is Rust and in-process — no conductor binary, no sandbox, no websocket handshake — which removes the entire failure class that blocks Tryorama, rather than working around it.

**Sequencing (deliberate).** Write the sweettest scenarios on the current pins *first*, then migrate to Holochain 0.7. Migrating first would revert MVP Phase 0's Verified status with no test suite able to say whether the migration broke a zome. Holochain 0.7.0 released 2026-07-30 and has no patch release yet, while 0.8.0-dev is already three releases in — further reason to land tests before chasing the version.
**Pending for promotion to Accepted:** sweettest scenarios (replacing the abandoned M13 Tryorama pin), action-time gating, DID/header hardening, cross-frame validation.

**Also pending, and the item that actually blocks others:** the consent anchor itself. `entry_has_consent()` only checks that `consent_ref.decision_action_hash` is a non-empty string — it never resolves the hash — so the governed gate fails closed on absence but accepts any value on presence. ADR-19's ratification is deferred on exactly this. Defining what `decision_action_hash` anchors to is the remaining ADR-12 design work; the sweettest port proves the zome, it does not settle the anchor.
