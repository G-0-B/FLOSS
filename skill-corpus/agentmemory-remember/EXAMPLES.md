# agentmemory-remember worked examples
...
User: "Save this to memory: the rate limiter in middleware/limit.ts counts per-IP, not per-user, so logged-in users on shared IPs collide."
...
User: "Remember this for later." (in a memory-search context, not a handoff note)
...
## 4. Disambiguating from the handoff-note `remember` skill

User: "Remember where we are before I close this session."

This is a cross-session handoff, not a tagged long-term insight — use the
`remember` skill (writes `.remember/remember.md`), not this one. If genuinely
ambiguous, ask which the user means before picking a skill.
