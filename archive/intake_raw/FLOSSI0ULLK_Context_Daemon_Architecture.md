...
This tiered model, inspired by the KAIROS three-layer memory architecture (lightweight index → topic files → raw transcripts, with nightly autoDream distillation) [file:209], allows agents to pay only for the context they actually need.
...
- **Delta-CRDTs** for conflict-free distributed state [web:286]
- **AD4M Perspectives** for semantic spanning across substrates [file:211]
- **Holochain Sourcechain** for provenance and sovereign agent history [file:211]
...
|------|---------------|-----------|------|
| **document-mcp** (yairwein, 2025) [web:238] | Watchdog + LanceDB + Ollama | Incremental re-embedding, SHA-256 fingerprinting, debouncing | No graph, no provenance |
| **DeepContext** (Wildcard AI, 2026) [web:258] | tree-sitter AST + pre-index | Symbol-aware, instant query | Code-only, no docs/ADRs |
| **Code Context Manager MCP** [web:252] | Redis RediSearch + SQLite + AST | Fast hybrid search | Not local-first, no delta-sync |
| **kairos-context-keeper** (turtir-ai, 2025) [web:240] | Neo4j + ChromaDB + FastAPI | Knowledge graph + vector, proactive daemon | Centralized, heavy dependencies |
| **Graphiti** (getzep, 2024–2026) [web:272] | Bi-temporal knowledge graph | Incremental updates, no full recomputation, MCP server | Requires external Neo4j |
| **memX** (MehulG, 2025) [web:244] | CRDT-style shared state + pub/sub | Real-time multi-agent sync, JSON Schema enforcement | No semantic layer |
| **DeltaStream** (2026) [web:250] | Apache Flink + ClickHouse + MCP | Stateful stream processing, full lineage | Enterprise/cloud-focused |
| **neuledge/context** (2026) [web:259][web:262] | Local-first doc layer | Lightweight, local-first design | Early-stage |
...
The leaked Claude Code internal design (as analyzed in the oh-my-meta.md integration report) reveals a **three-layer persistent memory** architecture that is the most production-battle-tested design available [file:209]:
...
The `autoDream` nightly consolidation process is the engine that keeps this coherent: it reconciles contradictions, promotes tentative `Unverified` observations to `Verified` facts, and prunes stale entries. This maps exactly onto the FLOSSI0ULLK **Claim Truth Model** (Verified / Specified / Aspirational / Unverified) [file:209].

The open-source community reverse-engineered KAIROS within days of the Claude Code source leak, producing at least one open reimplementation (`turtir-ai/kairos-context-keeper`) [web:240]. This validates the architecture and provides a starting point for implementation.

### 2.3 CRDTs as the Synchronization Primitive

A 2026 analysis from Zylos AI [web:286] establishes the case for CRDTs (Conflict-free Replicated Data Types) as the foundation for distributed multi-agent state. Key findings:

- **Delta-CRDTs** (pioneered by CBaquero [web:271][web:277]) exchange only the *diff* since last sync, not full replicas — directly addressing the resource intensity concern
...
- FOSDEM 2026 hosted its first dedicated "Local First, sync engines and CRDTs" devroom, signaling mainstream adoption [web:286]
...
Graphiti (getzep, Apache 2.0, 2024–2026) [web:272][web:275] is the most architecturally sophisticated open-source solution for agent memory management. Its key properties:
...
- **SHA gate:** Only re-embed if content hash changed since last indexing run (document-mcp pattern [web:238])
- **Chunking strategy:**
  - ADRs, specs → paragraph-level chunks with frontmatter preserved
  - Source code → function-level chunks via tree-sitter AST parsing (DeepContext pattern [web:258])
...
**Mechanism (Graphiti-inspired [web:272]):**
...
**Delta-sync protocol:** Agents exchange only the *delta* since their last vector clock timestamp [web:271][web:286]. An agent that goes offline and reconnects receives only the changes that occurred during its absence — not a full state dump. This is the fundamental efficiency gain over broadcast architectures.

**Gossip convergence:** Following the Honeybee Byzantine-tolerant peer sampling model [file:211], agents gossip state updates through a random subset of peers rather than broadcasting to all. This scales sublinearly with agent count.
...
**Four-phase cycle (KAIROS-inspired [file:209]):**
...
**Dual-prompt rendering (omo pattern [file:209]):**
...
This resolves the core architectural problem identified in oh-my-meta.md: giving a 1,100-line Claude prompt to a GPT model causes self-contradiction [file:209]. The Curator's translator role ensures every model receives context in its native cognitive format.
...
### Hook Integration (omo Seam 1 [file:209])
...
The five integration seams identified in oh-my-meta.md map directly to the Living Context Daemon's layers [file:209]:
...
AD4M's Perspectives (private local graph databases that can link data across protocols) are an **architectural natural fit** for Layer 2's Knowledge Graph [file:211]. An AD4M Perspective over the FLOSSI0ULLK workspace would provide: cryptographic provenance, protocol agnosticism, and native participation of AI agents as first-class members of the collective.

**The tension:** AD4M is more complex to operate than SQLite + Graphiti. It requires running an AD4M executor node.

**Proposed resolution:** Design Layer 2's Knowledge Graph with an **AD4M-compatible ontology from day one** — using the same triple structure (Agent → Language → Expression → Perspective) so that the graph can be migrated to AD4M when the team is ready. This is a "Now/Later" framing: build for SQLite/Graphiti **Now**, architect for AD4M **Later**.
...
The minimum viable daemon that proves the architecture:

1. **Observer** (Layer 0): Python watchdog watching `FLOSS/docs/adr/` and `INDEX.md`. SHA-256 gate. Logs change events to `_reference/context-daemon/events.jsonl`.
2. **Semantic Index** (Layer 1): LanceDB + nomic-embed-text. Index only `FLOSS/docs/`. Expose `query_context()` as a single MCP tool.
3. **Basic MCP server**: Python FastMCP. Two tools: `query_context` and `get_adr`. No graph yet.

**Verification test:** An agent starting a new session calls `query_context("What is the current consensus protocol decision?")` and receives an answer grounded in actual ADR content, without reading any files manually.

### Phase 1 — Knowledge Graph (2–4 weeks)

4. **Knowledge Graph** (Layer 2): Graphiti + SQLite. Ingest all ADRs as structured nodes. Add `claim_context()` MCP tool. Implement bi-temporal query support.
5. **Dual-prompt rendering**: Curator generates `CONTEXT-CLAUDE.md` and `CONTEXT-GPT.md` on each run. Add `get_agent_view()` MCP tool.

### Phase 2 — Shared State and Curator (4–8 weeks)

6. **CRDT state** (Layer 3): memX-inspired shared state for task queue and agent registry. Add `watch_changes()` MCP tool with SSE streaming.
7. **AutoDream Curator** (Layer 4): Full four-phase consolidation cycle. Scheduled nightly. Integrated with Graphiti for Claim Truth Model promotion.
8. **Consensus-gate hook** (Seam 1): Register with omo's hook system. Ternary voting via MetaCoordinator.

### Phase 3 — Distribution and AD4M Migration (ongoing)

9. **Multi-node CRDT gossip**: Extend delta-sync to work across multiple developer machines.
10. **AD4M Perspective migration**: Migrate Knowledge Graph to AD4M for full cryptographic provenance and inter-agent participation.
11. **Meta Harness optimization**: Treat daemon configuration (tiering policies, batch windows, embedding model selection, Curator LLM model) as harness parameters subject to automated optimization.

---

## Conclusion: The Living Context as Infrastructure

The Living Context Daemon is not a feature — it is **infrastructure**. Like TCP/IP, it should be invisible to the agents using it while making everything they do more coherent, more trustworthy, and more compounding. An agent that calls `query_context("what did we decide about the authentication approach?")` and gets back a provenance-tracked, bi-temporally accurate answer in its native prompt format is an agent that can spend its reasoning budget on the actual problem rather than on reconstructing shared history.

The architecture proposed here is composable in the FLOSSI0ULLK sense: each layer can be replaced or upgraded independently. The Observer can swap watchdog for inotify bindings. The Semantic Index can upgrade from local to API embeddings. The Knowledge Graph can migrate from SQLite to Neo4j to AD4M. The CRDT layer can evolve from a single-machine in-memory store to a Holochain DHT-backed distributed state. None of these migrations require rebuilding the MCP API surface that agents depend on.

The first step is the Walking Skeleton: a watchdog watching ADRs, a LanceDB index over the docs corpus, and two MCP tools. That 200-line implementation is the seed crystal from which the full Living Context Daemon grows. Build it. Run it. Let it watch.

---

*This report is a Harvest Packet for the FLOSSI0ULLK / Amazon Rose Forest ecosystem.*
*Claim Truth Model status: Specified (architecture) / Aspirational (Phase 2+)*
*Next action: File as ADR-CONTEXT-DAEMON, assign Phase 0 implementation task to Hephaestus or equivalent deep worker.*
